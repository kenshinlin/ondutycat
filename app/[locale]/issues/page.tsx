...
const mockIssues: Issue[] = [
  {
    ...
    description: 'Multiple alerts indicating database connection pool utilization exceeding 90% threshold',
    status: 'resolved',
    isRealIssue: true,
    aiConclusion: 'Confirmed connection pool exhaustion. Root cause identified as unclosed connections in payment service. Recommended increasing pool size from 50 to 100 and fixing connection leak in payment-processing module.',
    skillId: 'skill-001',
    ...
    logs: [
      {
        ...
        content: 'Matched skill: Database Connection Failure (confidence: 0.94)',
      ...
      },
      {
        id: 'log-002',
        logType: 'tool_called',
        content: 'Called Database_Health_Check: Connection pool at 95% (47/50 connections active)',
        metadata: { toolName: 'Database_Health_Check', result: { activeConnections: 47, poolSize: 50 } },
        createdAt: '2025-01-15T10:31:30Z',
      },
      {
        id: 'log-003',
        logType: 'tool_called',
        content: 'Called Log_Analyzer: Found 15 connection timeout errors in payment-service logs',
        metadata: { toolName: 'Log_Analyzer', errorCount: 15, service: 'payment-service' },
        createdAt: '2025-01-15T10:32:00Z',
      },
      {
        id: 'log-004',
        logType: 'reasoning',
        content: 'Analysis indicates connection leak in payment-service. The service creates connections but fails to close them in error scenarios. This matches pattern of connection pool exhaustion.',
        createdAt: '2025-01-15T10:35:00Z',
      },
      {
        id: 'log-005',
        logType: 'conclusion',
        content: 'REAL ISSUE: Database connection pool exhaustion caused by connection leak in payment-service. Immediate actions: 1) Increase pool size, 2) Fix connection handling in payment-processing module',
    ...
    logs: [
      {
        id: 'log-006',
        logType: 'skill_matched',
        content: 'Matched skill: API Response Time Degradation (confidence: 0.89)',
      ...
      },
      {
        id: 'log-007',
        logType: 'tool_called',
        content: 'Called API_Check: Checkout endpoint showing 3.2s average response time',
        metadata: { toolName: 'API_Check', endpoint: '/checkout', avgResponseTime: 3200 },
  ...
  },
  {
    ...
    description: 'CPU usage spike detected on backup server during maintenance window',
    status: 'false_positive',
    isRealIssue: false,
    aiConclusion: 'False positive. CPU spike occurred during scheduled backup window (02:00-04:00 UTC). This is expected behavior and does not require action.',
    skillId: 'skill-004',
    ...
    logs: [
      ...
      },
      {
        ...
        metadata: { toolName: 'Process_Monitor', processName: 'backup-job', cpuUsage: 85 },
      ...
      },
      {
        id: 'log-012',
        logType: 'conclusion',
        content: 'FALSE POSITIVE: CPU spike is from scheduled backup job running during maintenance window. This is expected behavior.',
  ...
  },
  {
    ...
    isRealIssue: null,
    aiConclusion: 'Unable to determine root cause. External payment gateway API is failing. No access to external service metrics. Recommended: Contact payment gateway provider and check their status page.',
    skillId: 'skill-003',
    skillName: 'API Response Time Degradation',
    createdBy: 'system',
    ...
    logs: [
      {
        id: 'log-014',
        logType: 'skill_matched',
        content: 'Matched skill: API Response Time Degradation (confidence: 0.82)',
      ...
      },
      {
        id: 'log-015',
        logType: 'tool_called',
        content: 'Called API_Check: External payment gateway returning 503 Service Unavailable',
        metadata: { toolName: 'API_Check', endpoint: 'payment-gateway-api.com', statusCode: 503 },
        createdAt: '2025-01-14T18:01:30Z',
      },
      {
        id: 'log-016',
        logType: 'reasoning',
        content: 'External API failing. Checking if we have visibility into payment gateway infrastructure...',
        createdAt: '2025-01-14T18:15:00Z',
      },
      {
        id: 'log-017',
        logType: 'conclusion',
        content: 'UNABLE TO RESOLVE: External payment gateway is failing. No access to provider infrastructure. Recommend escalating to payment gateway support team.',
        metadata: { externalService: 'payment-gateway-api.com', actionRequired: 'contact-provider' },
...
const statusConfig: Record<
  IssueStatus,
  { variant: 'success' | 'warning' | 'danger' | 'info' | 'default'; label: string; icon: typeof CheckCircle2 }
> = {
  ...
  false_positive: { variant: 'default', label: 'False Positive', icon: XCircle },
  unable_to_resolve: { variant: 'danger', label: 'Unable to Resolve', icon: AlertCircle },
};
...
export default function IssuesPage() {
  ...
  const filteredIssues = mockIssues.filter((issue) => {
    const matchesSearch =
      ...
      (issue.skillName && issue.skillName.toLowerCase().includes(searchQuery.toLowerCase()));
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">AI-processed alerts with analysis</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            <div className="text-center">
              <div className="text-2xl font-bold text-foreground">{filteredIssues.length}</div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          {issue.isRealIssue !== null && (
                            <Badge variant={issue.isRealIssue ? 'success' : 'default'}>
                              {issue.isRealIssue ? 'Real Issue' : 'False Positive'}
                            </Badge>
                          )}
                        </div>
                        <h3 className="text-base font-semibold text-card-foreground mb-1">{issue.title}</h3>
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    {issue.aiConclusion && (
                      <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-3">
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-medium text-blue-600 mb-1">AI Conclusion</p>
                            <p className="text-sm text-gray-700">{issue.aiConclusion}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {issue.skillName && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-muted-foreground">Matched Skill:</span>
                        <span className="font-medium text-card-foreground">{issue.skillName}</span>
                      </div>
                    )}
                    ...
                    <div className="space-y-2">
                      <p className="text-xs font-medium text-muted-foreground">
                        {issue.alerts.length} Related Alert{issue.alerts.length > 1 ? 's' : ''}
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {issue.alerts.map((alert) => (
                          ...
                          >
                            ...
                            <span className="truncate max-w-[150px]">{alert.message}</span>
                          </div>
                        ))}
                      </div>
                    </div>
